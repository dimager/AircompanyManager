        ...
        <div class="col">
            ...
            <div class="form-group">
                <div>
                    <c:choose>
                        <c:when test="${requestScope.editpage}">
                            <h3 id="heading"><fmt:message bundle="${lang}" key="formname.editaircraft"/></h3>
                        </c:when>
                        <c:otherwise>
                            <h3 id="heading"><fmt:message bundle="${lang}" key="formname.addaircraft"/></h3>
                        </c:otherwise>
                    </c:choose>
                </div>
                <c:out value="${requestScope.command}"></c:out>
                <label><fmt:message bundle="${lang}" key="label.regcode"/></label>
                ...
                <label><fmt:message bundle="${lang}" key="label.producer"/></label>
                ...
                <label><fmt:message bundle="${lang}" key="label.model"/></label>
                    ...
                    <c:choose>
                        <c:when test="${fn:contains(pageContext.request.queryString,'edit_aircraft_id')}">
                            ...
                            <button class="btn btn-primary" id="button" style="background-color:#007ac9;" type="submit">
                                <fmt:message bundle="${lang}" key="button.editaircraft"/></button>
                        </c:when>
                        <c:otherwise>
                            <input type="hidden" name="command" value="ADD_AIRCRAFT"/>
                            <button class="btn btn-primary" id="button" style="background-color:#007ac9;" type="submit">
                                <fmt:message bundle="${lang}" key="button.addaircraft"/></button>
                        </c:otherwise>
                    </c:choose>
                </div>
            </div>
