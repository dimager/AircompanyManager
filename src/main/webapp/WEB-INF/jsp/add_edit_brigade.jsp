        ...
        <div class="col">
            ...
            <div class="form-group">
                <div>
                    <c:choose>
                        <c:when test="${requestScope.editpage}">
                            <h3 id="heading"><fmt:message bundle="${lang}" key="formname.editbrigade"/></h3>

                        </c:when>
                        <c:otherwise>
                            <h3 id="heading"><fmt:message bundle="${lang}" key="formname.addbrigade"/></h3>
                        </c:otherwise>
                    </c:choose>
                </div>
                <label><fmt:message bundle="${lang}" key="label.brigadename"/></label>
                ...
                <c:if test="${not empty requestScope.brigadeDTO}">
                       ...
                       placeholder="<fmt:message bundle="${lang}" key="label.brigadename"/>" name="brigadename" minlength="1"
                       maxlength="50">
                    ...
                    <c:choose>
                        <c:when test="${requestScope.editpage}">
                            ...
                            <button class="btn btn-primary" id="button" style="background-color:#007ac9;" type="submit">
                                <fmt:message bundle="${lang}" key="buttonname.editbrigade"/></button>
                        </c:when>
                        <c:otherwise>
                            <input type="hidden" name="command" value="ADD_BRIGADE">
                            <button class="btn btn-primary" id="button" style="background-color:#007ac9;" type="submit">
                                <fmt:message bundle="${lang}" key="buttonname.addbrigade"/></button>
                        </c:otherwise>
                    </c:choose>
                </div>
            </div>
